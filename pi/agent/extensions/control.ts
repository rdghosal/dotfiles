...
Messages automatically include sender session info for replies. When you want a response, instruct the target session to reply directly to the sender by calling send_to_session with the sender_info reference (do not poll get_message).`,
    parameters: Type.Object({
      ...
      message: Type.Optional(
        Type.String({ description: "Message to send (required for action=send)" })
      ),
      mode: Type.Optional(
      ...
      try {
        ...
        if (action === "get_message") {
          ...
          if (!result.response.success) {
            return {
              content: [
                { type: "text", text: `Failed: ${result.response.error ?? "unknown error"}` },
              ],
              isError: true,
              details: result,
            };
        ...
        return {
          content: [
            { type: "text", text: `Message sent to session ${displayTarget || targetSessionId}` },
          ],
          details: result.response.data,
        };
      } catch (error) {
