...
function createSession(cwd: string): void {
  exec(
    tmux(["new-session", "-d", "-s", SESSION_NAME, "-n", "make", "-c", `'${shellEscape(cwd)}'`])
  );
}
...
async function runMakeInSplitPane(
  ...
  const result = await execFn("tmux", ["split-window", "-h", "-c", ctx.cwd, "make", target], {
    timeout: 5000,
  });
...
async function runMakeInBackgroundSession(
  ...
  try {
    ...
    exec(
      tmux(["send-keys", "-t", windowTarget, "-l", "--", `'make ${shellEscape(target)}'`]),
      3000
    );
...
async function handleView(target: string | undefined, ctx: ExtensionCommandContext): Promise<void> {
  ...
  const windowTarget = target ? `${SESSION_NAME}:${sanitizeWindowName(target)}` : SESSION_NAME;
