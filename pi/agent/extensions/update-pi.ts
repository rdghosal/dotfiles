...
async function checkForUpdate(pi: ExtensionAPI): Promise<{ current?: string; latest?: string; hasUpdate: boolean }> {
...
async function performUpdate(pi: ExtensionAPI, signal?: AbortSignal): Promise<UpdateResult> {
	...
	try {
		const result = await pi.exec(
			"npm",
			["update", "-g", "@mariozechner/pi-coding-agent"],
...
export default function updatePiExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("update", {
		...
		handler: async (_args, ctx) => {
			...
			if (result.success) {
				const versionMsg = result.newVersion && result.newVersion !== result.previousVersion
	...
	pi.registerTool({
		name: "update_pi",
		label: "Update Pi",
		description: "Update pi to the latest version from npm. Checks current version, fetches the latest, and performs the update if needed.",
