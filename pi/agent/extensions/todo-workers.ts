...
const TodoWorkerParams = Type.Object({
  ...
  force: Type.Optional(
    Type.Boolean({ description: "Skip confirmation prompt for destructive operations" })
  ),
});
...
async function handleStart(
  ...
  try {
    ...
    const newPane =
      execSync("tmux list-panes -F '#{pane_id}'", { encoding: "utf-8" })
        .trim()
        .split("\n")
        .pop()
        ?.trim() || "unknown";
...
async function handleStop(
  ...
  if (!force) {
    ...
    const confirmed = ctx.hasUI ? await ctx.ui.confirm("Stop worker?", message) : true; // Non-interactive mode defaults to proceeding
    if (!confirmed) {
  ...
  try {
    ...
    // Remove worktree
    execSync(`git worktree remove ${shellEscape(worker.worktree)}`, {
      cwd: gitRoot,
      encoding: "utf-8",
    });
...
async function handleLayout(gitRoot: string): Promise<ToolResult> {
  ...
  try {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      execSync(`tmux send-keys -t ${i} ${shellEscape(`cd ${info.worktree} && pi`)} Enter`, {
        encoding: "utf-8",
      });
    }
    ...
    return {
      content: [{ type: "text", text: `Laid out ${entries.length} workers in tiled layout` }],
    };
  } catch (e) {
    const error = e instanceof Error ? e.message : String(e);
...
export default function todoWorkersExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (!process.env.TMUX)
        return errorResult("Error: Not in a tmux session. Cannot manage panes.");
